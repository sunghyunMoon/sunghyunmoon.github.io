...
- 예를 들어 어떤 함수가 A I B 타입의 매개변수를 받는다고 가정해보자. 인자 타입이 A 또는 B 일 때를 구분해서 로직을 처리하고 싶다면 어떻게 해야 할까?
...
- 종중 태그되 유니온 으로도 불리는 식별할 수 있는 유니온은 타입 좁히기에 널리 사용되는 방식이다. 이 절에서는 예시를 살펴보면서 식별할 수 있는 유니온을 어떨 때 사용할 수 있는지와 장점을 알아본다.
